...
namespace SampleCode
{
    ...
    public class RectangleViewModel : INotifyPropertyChanged
    {
        ...
        public void colorConvolution(double[, ,] MC, double _red, double _blue, double _green)
        {
            ...
            for (int y = 1; y < BImage.PixelHeight - 1; y++)
            {
                for (int x = 1; x < BImage.PixelWidth - 1; x++)
                {
                    ...
                    pixels[index] = (byte)(array[x, y].Blue * _blue);
                    pixels[index + 1] = (byte)(array[x, y].Green * _green);
                    pixels[index + 2] = (byte)(array[x, y].Red * _red);
                    pixels[index + 3] = 255;
                }
            }
        ...
        public void imageSelectionDeletion(int topLeft, int topRight, int bottomLeft, int bottomRight)
        {
            ...
            // Width
            int calcWidth = System.Math.Abs(topLeft - topRight);
            // Height
            int calcHeight = System.Math.Abs(topLeft - bottomLeft);
            // New Array
            ...
            for (int y = topLeft; y < bottomLeft; y++)
            {
                for (int x = topLeft; x < topRight; x++)
                {
                    int index = (y - topLeft) * (calcWidth * 4) + 4 * (x - topLeft);
        ...
        public String imageSelectionRetrivial(int topLeft, int topRight, int bottomLeft, int bottomRight)
        {
