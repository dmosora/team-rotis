...
namespace SampleCode
{
    ...
    public class ViewModel : INotifyPropertyChanged
    {
        ...
        public RectangleViewModel addRectangle(double x, double y,double width, double height)
        {
            ...
            var rtemp = new RectangleViewModel(x, y, bitemp.PixelWidth, bitemp.PixelHeight, bitemp, opac, "Drawing " + (rectangles.Count() + 1).ToString());
            rectangles.Add(rtemp);
            bitemp.UriSource = new Uri("", UriKind.Relative);
        ...
        public void copy(RectangleViewModel[] sender)
        {
            ...
            foreach (RectangleViewModel RVM in sender)
            {
                if (RVM != null)
                {
                    ...
                    copied[j] = new RectangleViewModel(RVM.X - leftbound, RVM.Y - topbound, RVM.Width, RVM.Height, RVM.BImage, RVM.Opacity, RVM.RectName + " Copy");
        ...
        public void paste(Point mousePosition)
        {
            for (int j = 0; j < copied.Length; j++)
            {
                if (copied[j] != null)
                {
                    ...
                    var rtemp = new RectangleViewModel(copied[j].X + mousePosition.X, copied[j].Y + mousePosition.Y - 25, copied[j].Width, copied[j].Height, copied[j].BImage, copied[j].Opacity, copied[j].RectName + numberCopied);
        ...
        public void moveUp(RectangleViewModel sender)
        {
            int start = rectangles.IndexOf(sender);
            rectangles[start] = rectangles[start + 1];
            rectangles[start+1] = sender;
        }

        public void moveDown(RectangleViewModel sender)
        {
            int start = rectangles.IndexOf(sender);
            rectangles[start] = rectangles[start - 1];
            rectangles[start - 1] = sender;
            ...
            Geometry tgeometry = text.BuildGeometry(start);
             GeometryDrawing textHolder =
                new GeometryDrawing(new SolidColorBrush(fillColor),
                    new Pen(new SolidColorBrush(fillColor), 1),
                    tgeometry
                );
